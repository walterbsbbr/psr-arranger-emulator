...
int main(int argc,char**argv){
    ...
    // Count notes per channel in first section (Main A usually)
    bool inMainA=false;
    std::map<int,std::map<int,int>> chNoteCount; // ch -> noteClass -> count
    int firstNotes=0;
    while(pos<tSz){
        ...
        if(st==0xFF){uint8_t t=trk[pos++];uint32_t l=readVlq(trk,tSz,pos);
            if(t==0x06){std::string txt((char*)trk+pos,l);
                if(txt=="Main A")inMainA=true;
                else if(inMainA && txt!="Main A"){inMainA=false;break;}
            }
            pos+=l;
        }else if(st==0xF0){uint32_t l=readVlq(trk,tSz,pos);pos+=l;}
        else{uint8_t type=st&0xF0;uint8_t ch=st&0x0F;
            if(type==0x90&&inMainA){uint8_t note=trk[pos],vel=trk[pos+1];
                if(vel>0){chNoteCount[ch][note%12]++;firstNotes++;}
                pos+=2;
            }else if(type==0x80){pos+=2;}
            else if(type==0xC0||type==0xD0){pos++;}
            else{pos+=2;}
        }
        if(firstNotes>500)break;
    }
    printf("=== Note analysis for Main A section ===\n");
    for(auto&[ch,notes]:chNoteCount){
        printf("Ch %2d: ",ch+1);
        int maxCount=0,maxNote=0;
        for(auto&[nc,cnt]:notes){if(cnt>maxCount){maxCount=cnt;maxNote=nc;}}
        for(auto&[nc,cnt]:notes)printf("%s=%d ",nn[nc],cnt);
        printf(" (most: %s)\n",nn[maxNote]);
    }
    return 0;
}
